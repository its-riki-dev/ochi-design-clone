...
const Navbar = () => {
    ...
    return (
        <div
            className={`fixed z-[999] w-full px-13 py-5 flex justify-between items-center backdrop-blur-xs bg-white/3 text-black transition-transform duration-300 ${!showNavbar ? '-translate-y-full' : ''
                }`}
        >
            ...
            <div className="links flex gap-10">
                {['Services', 'Our Work', 'About Us', 'Insights', 'Contact Us'].map((item, index) => (
                    <a
                        key={index}
                        className={`text-md capitalize font-light font-["NeueMontreal"] relative ${index === 4 && 'ml-70'
                            } group cursor-pointer`}
                    >
                        {item}
                        <span className="absolute left-0 bottom-0 block w-0 h-[1px] bg-black transition-all duration-300 group-hover:w-full group-hover:left-0 group-hover:right-0"></span>
                    </a>
                ))}
            </div>
        </div>
